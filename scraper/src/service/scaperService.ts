// src/service/scrapeService.ts
import puppeteer from 'puppeteer';
import { IHotelConfig, IScrapedPrice } from '../models/interfaces/Interfaces';
import { dateUtils } from '../utils';
import { utils } from './regensburg/includio/utils';

const scrapeData = async (hotel: IHotelConfig, arrivalDate: Date, daysAhead: number): Promise<IScrapedPrice[]> => {
...
export const scrapeHotelData = async (hotel: IHotelConfig): Promise<IScrapedPrice[]> => {
    const today = new Date();
    const nextMonth = dateUtils.addMonths(today, 1);

    const pricesToday = await scrapeData(hotel, today, 1);  // Heute + 1 Tag
    const pricesNextMonth = await scrapeData(hotel, nextMonth, 1);  // Nächster Monat + 1 Tag
